...
# =============================================================================
ticker = 'NLOK' # security to evaluate
date = '2020-01-13' #evaluation date
...
iVol_obj_vendor_2 = ImpliedVolatilityCalculator(root_finder_method,
                                                model_name,
                                                ZC_curve_vendor,
...
model_prices_vendor_2 = iVol_obj_vendor_2.calculateImpliedVol()



vol_surface_obj_vendor_2 = gaussian_smooth()




vol_surface_vendor_2 = vol_surface_obj_vendor_2.generate_volatility_surface(model_prices_vendor_2)


print(vol_surface_vendor_2.head())
...
db_pandora = DatabaseAccess.get_instance_of_DBaccess('Pandora', 
                                                     'Pandora', 
                                                     'EJV_Derivs')
# testing database connection
securityID_ejv = db_pandora.get_securityid_from_ticker(ticker) # getting secutiry ID
securityID_pandora =  db_pandora.get_securityid_from_ticker(ticker, 'ORACLE') # getting secutiry ID
div_method_pandora = constant()


div_list_pandora = db_pandora.get_dividend_list(securityID_ejv ,date)
spot_pandora = db_pandora.get_spot_from_securityID_date(securityID_pandora ,date)
...
dividend_yield_pandora_2 = db_pandora.get_dividend_yield(securityID_pandora,date)   
                                                                                       # dividend calculation
ZC_curve_pandora = db_pandora.get_ZCcurve_replication(date)
option_prices_pandora = db_pandora.pull_optionprice_database(securityID_ejv, date)
